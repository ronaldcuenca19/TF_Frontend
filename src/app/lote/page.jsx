...
import { all_lote_producto, all_product_caducado, all_product_fresco, all_product_por_caducar } from "@/src/hooks/service_producto";
...
export default function Page() {
    ...
    const [caducado, setCaducado] = useState([]);
    const [apunto, setAPunto] = useState([]);
    const [fresco, setFresco] = useState([]);
    const [obt, setObt] = useState(false);

    useEffect(() => {
        if (!obt) {
            all_lote_producto().then((info) => {
                console.log(info);
                if (info.code === 200) {
                    setLote(info.datos);
                    setObt(true);
